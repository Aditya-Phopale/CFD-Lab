...
void FreeSurfaceBoundary::apply_black(Fields &field, Grid &grid) {
    ...
    for (auto cells : _cells) {
        ...
        if (cells->is_border(border_position::RIGHT)) {
            ...
            //     cell_type::EMPTY) {
            //     field.v(i + 1, j - 1) = field.v(i, j - 1) - grid.dx() * (field.u(i, j) - field.u(i, j - 1)) /
            //     grid.dy();
            // }
            field.f(i, j) = field.u(i, j);
        ...
        if (cells->is_border(border_position::TOP)) {
            ...
            // {
            //     field.u(i - 1, j + 1) = field.u(i - 1, j) - grid.dy() * (field.v(i, j) - field.v(i - 1, j)) /
            //     grid.dx();
            // }
            field.g(i, j) = field.v(i, j);
        ...
        if (cells->is_border(border_position::SOUTHEAST)) {
            ...
            //     cell_type::EMPTY) {
            //     field.u(i - 1, j - 1) =
            //         field.u(i, j) + grid.dy() * (field.v(i, j - 1) - field.v(i - 1, j - 1)) / grid.dx();
            // }
            // if (cells->neighbour(border_position::RIGHT)->neighbour(border_position::TOP)->type() ==
            // cell_type::EMPTY) {
