...
void MovingWallBoundary::apply(Fields &field) {
    ...
    for (auto cells : _cells) {
        ...
        j = cells->j();
        if (cells->is_border(border_position::BOTTOM)) {
            field.u(i, j) = (2.0) * (_wall_velocity[LidDrivenCavity::moving_wall_id]) - field.u(i, j - 1);
            field.u(i - 1, j) = (2.0) * (_wall_velocity[LidDrivenCavity::moving_wall_id]) - field.u(i - 1, j - 1);
            field.v(i, j - 1) = 0.0;
            field.g(i, j - 1) = field.v(i, j - 1);
            continue;
        }
        ...
        if (_cells[i]->is_border(border_position::RIGHT)) {
            field.u(i, j) = 0.0;
            field.v(i, j) = (2.0) * (_wall_velocity[LidDrivenCavity::moving_wall_id]) - field.v(i + 1, j);
            field.f(i, j) = field.u(i, j);
            continue;
        }
...
OutletBoundary::OutletBoundary(std::vector<Cell *> cells, std::map<int, double> wall_temperature)
    : _cells(cells), _wall_temperature(wall_temperature) {}
...
void FreeSlipBoundary::apply_pressure(Fields &field) {
    int i, j;
    for (auto cells : _cells) {
        i = cells->i();
        j = cells->j();
...
InletBoundary::InletBoundary(std::vector<Cell *> cells, double uin, double vin) : _cells(cells), _uin(uin), _vin(vin) {}

InletBoundary::InletBoundary(std::vector<Cell *> cells, double uin, double vin, std::map<int, double> wall_temperature)
...
AdiabaticBoundary::AdiabaticBoundary(std::vector<Cell *> cells) : _cells(cells) {}
...
FreeSurfaceBoundary::FreeSurfaceBoundary(std::vector<Cell *> cells) : _cells(cells) {}

void FreeSurfaceBoundary::apply_black(Fields &field, Grid &grid) {
    int i, j;
    for (auto cells : _cells) {
        i = cells->i();
        j = cells->j();
        ...
        if (cells->is_border(border_position::NORTHSOUTH)) {
            field.v(i, j) = field.v(i, j) + field.gy() * field.dt();
...
void FreeSurfaceBoundary::apply_pressure(Fields &field, Grid &grid) {
    int i, j;
    for (auto cells : _cells) {
        i = cells->i();
        j = cells->j();
        if (cells->is_border(border_position::RIGHT)) {
            field.p(i, j) = 2 * (field.u(i, j) - field.u(i - 1, j)) / (field.Re() * grid.dx());
            continue;
        }
        if (cells->is_border(border_position::TOP)) {
            field.p(i, j) = 2 * (field.v(i, j) - field.v(i, j - 1)) / (field.Re() * grid.dy());
            continue;
        }
        if (cells->is_border(border_position::LEFT)) {
            field.p(i, j) = 2 * (field.u(i, j) - field.u(i - 1, j)) / (field.Re() * grid.dx());
        ...
        if (cells->is_border(border_position::SOUTHEAST)) {
            field.p(i, j) =
                (-0.5 / field.Re()) *
                ((field.u(i, j + 1) + field.u(i - 1, j + 1) - field.u(i, j) - field.u(i - 1, j)) / grid.dy() +
                 (field.v(i, j) + field.v(i, j - 1) - field.v(i - 1, j) - field.v(i - 1, j - 1)) / grid.dx());
            continue;
        }
        ...
        if (cells->is_border(border_position::NORTHEAST)) {
            field.p(i, j) =
                (0.5 / field.Re()) *
                ((field.u(i, j) + field.u(i - 1, j) - field.u(i, j - 1) - field.u(i - 1, j - 1)) / grid.dy() +
                 (field.v(i, j) + field.v(i, j - 1) - field.v(i - 1, j) - field.v(i - 1, j - 1)) / grid.dx());
            continue;
        }
        if (cells->is_border(border_position::SOUTHWEST)) {
            field.p(i, j) =
                (0.5 / field.Re()) *
                ((field.u(i, j + 1) + field.u(i - 1, j + 1) - field.u(i, j) - field.u(i - 1, j)) / grid.dy() +
                 (field.v(i + 1, j) + field.v(i + 1, j - 1) - field.v(i, j) - field.v(i, j - 1)) / grid.dx());
            continue;
        }
        if (cells->is_border(border_position::NORTHWEST)) {
            field.p(i, j) =
        ...
        if (cells->is_border(border_position::EASTWEST) || cells->is_border(border_position::NORTHSOUTH) ||
            cells->is_border(border_position::NORTHEASTWEST) || cells->is_border(border_position::NORTHEASTSOUTH) ||
            cells->is_border(border_position::NORTHWESTSOUTH) || cells->is_border(border_position::EASTWESTSOUTH) ||
            cells->is_border(border_position::NORTHEASTWESTSOUTH)) {
            field.p(i, j) = 0;
            continue;
        }
    }
...
void FreeSurfaceBoundary::apply_grey(Fields &field, Grid &grid) {
    ...
    for (auto cells : _cells) {
        ...
        if (cells->is_border(border_position::SOUTHEAST)) {
            ...
            if (cells->neighbour(border_position::RIGHT)->neighbour(border_position::TOP)->type() == cell_type::EMPTY) {
                field.v(i + 1, j) = field.v(i, j) - grid.dx() * (field.u(i, j + 1) - field.u(i, j)) / grid.dy();
                field.g(i + 1, j) = field.v(i + 1, j);
            }
        ...
        if (cells->is_border(border_position::NORTHWEST)) {
            if (cells->neighbour(border_position::TOP)->neighbour(border_position::RIGHT)->type() == cell_type::EMPTY) {
                field.u(i, j + 1) = field.u(i, j) - grid.dy() * (field.v(i + 1, j) - field.v(i, j)) / grid.dx();
                field.f(i, j + 1) = field.u(i, j + 1);
            }
            if (cells->neighbour(border_position::LEFT)->neighbour(border_position::BOTTOM)->type() ==
                cell_type::EMPTY) {
        ...
        if (cells->is_border(border_position::SOUTHWEST)) {
            if (cells->neighbour(border_position::TOP)->neighbour(border_position::LEFT)->type() == cell_type::EMPTY) {
                field.v(i - 1, j) = field.v(i, j) + grid.dx() * (field.u(i - 1, j + 1) - field.u(i - 1, j)) / grid.dy();
                field.g(i - 1, j) = field.v(i - 1, j);
            }
            if (cells->neighbour(border_position::RIGHT)->neighbour(border_position::BOTTOM)->type() ==
                cell_type::EMPTY) {
                field.u(i, j - 1) = field.u(i, j) + grid.dy() * (field.v(i + 1, j - 1) - field.v(i, j - 1)) / grid.dx();
                field.f(i, j - 1) = field.u(i, j - 1);
            }
