...
Case::Case(std::string file_name, int argn, char **args) {
    ...
    if (Communication::rank == 0) {
        ...
        MPI_Status status;
        MPI_Recv(rec_data.data(), 10, MPI_INT, 0, Communication::rank, MPI_COMM_WORLD, &status);
    ...
    if (not _grid.inlet_cells().empty()) {
        _boundaries.push_back(std::make_unique<InletBoundary>(_grid.inlet_cells(), UIN, VIN));
    }
    ...
    if (not _grid.adiabatic_cells().empty()) {
        _boundaries.push_back(std::make_unique<AdiabaticBoundary>(_grid.adiabatic_cells()));
    }
...
void Case::output_vtk(int timestep, int rank) {
    ...
    std::string ptcl_output_name =
        _dict_name + '/' + _case_name + "_" + std::to_string(rank) + "_" + std::to_string(timestep) + ".vtp";
    ...
    for (auto &particle : _grid.particle()) {
        ...
        pos[3] = std::sqrt(particle.u() * particle.u() + particle.v() * particle.v());
    ...
    std::string outputname =
        _dict_name + '/' + _case_name + "_" + std::to_string(rank) + "_" + std::to_string(timestep) + ".vtk";
