...
Fields::Fields(double nu, double Re, double alpha, double beta, double dt, double tau, int imax, int jmax, double UI,
    ...
    : _nu(nu), _Re(1 / nu), _dt(dt), _tau(tau), _alpha(alpha), _beta(beta), _gx(GX), _gy(GY), _energy_eq(energy_eq) {
...
void Fields::calculate_fluxes(Grid &grid) {
    ...
    for (auto cell : grid.surface_cells()) {
        ...
        } else if ((cell->is_border(border_position::NORTHWEST)) || (cell->is_border(border_position::TOP)) ||
                   (cell->is_border(border_position::NORTHWESTSOUTH))) {
            if (cell->neighbour(border_position::RIGHT)->type() == cell_type::FLUID ||
                cell->neighbour(border_position::RIGHT)->type() == cell_type::SURFACE) {
                _F(i, j) = _U(i, j) + _dt * (_nu * Discretization::diffusion(_U, i, j) -
                                             Discretization::convection_u(_U, _V, i, j) + _gx);
...
void Fields::calculate_temperature(Grid &grid) {
    ...
    for (auto cell : grid.fluid_cells()) {
        ...
        _T(i, j) = T_old(i, j) + _dt * (_alpha * Discretization::diffusion(T_old, i, j) -
