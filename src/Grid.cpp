...
Grid::Grid(std::string geom_name, Domain &domain) {
    ...
    if (geom_name.compare("NONE")) {
        ...
        geometry_excluding_ghosts.resize(_domain.size_x, std::vector<int>(_domain.size_y));
        for (int j = 1; j < _domain.size_y + 1; j++) {
            for (int i = 1; i < _domain.size_x + 1; i++) {
                geometry_excluding_ghosts.at(i - 1).at(j - 1) = geometry_data.at(i + domain.imin).at(j + domain.jmin);
            }
        }
...
void Grid::build_lid_driven_cavity() {
    ...
    geometry_excluding_ghosts.resize(_domain.domain_size_x, std::vector<int>(_domain.domain_size_y));
...
void Grid::assign_cell_types(std::vector<std::vector<int>> &geometry_data) {
    ...
    for (int j_geom = _domain.jmin; j_geom < _domain.jmax; ++j_geom) {
        ...
        for (int i_geom = _domain.imin; i_geom < _domain.imax; ++i_geom) {
            if (geometry_data.at(i_geom).at(j_geom) == cellID::fluid) {
                ...
                if (i_geom == _domain.imin || i_geom == _domain.imax - 1 || j_geom == _domain.jmin ||
                    j_geom == _domain.jmax - 1) {
                ...
                _cells(i, j) = Cell(i, j, cell_type::FIXED_WALL3, geometry_data.at(i_geom).at(j_geom));

                _fixed_wall_cells.push_back(&_cells(i, j));

            } else if (geometry_data.at(i_geom).at(j_geom) == cellID::fixed_wall_4) {
                _cells(i, j) = Cell(i, j, cell_type::FIXED_WALL4, geometry_data.at(i_geom).at(j_geom));

                _fixed_wall_cells.push_back(&_cells(i, j));

            } else if (geometry_data.at(i_geom).at(j_geom) == cellID::fixed_wall_5) {
                _cells(i, j) = Cell(i, j, cell_type::ADIABATIC_WALL, geometry_data.at(i_geom).at(j_geom));
                ...
                _cells(i, j) = Cell(i, j, cell_type::OUTLET, geometry_data.at(i_geom).at(j_geom));
                ...
                _cells(i, j) = Cell(i, j, cell_type::EMPTY, geometry_data.at(i_geom).at(j_geom));
            } else if (geometry_data.at(i_geom).at(j_geom) == cellID::free_slip) {
    ...
    if (_cells(i, j).neighbour(border_position::TOP)->type() == cell_type::FLUID ||
        _cells(i, j).neighbour(border_position::TOP)->type() == cell_type::SURFACE ||
        _cells(i, j).neighbour(border_position::TOP)->type() == cell_type::EMPTY) {
        _cells(i, j).add_border(border_position::TOP);
    }
    if (_cells(i, j).neighbour(border_position::RIGHT)->type() == cell_type::FLUID ||
        _cells(i, j).neighbour(border_position::RIGHT)->type() == cell_type::SURFACE ||
        _cells(i, j).neighbour(border_position::RIGHT)->type() == cell_type::EMPTY) {
    ...
    if (_cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::FLUID ||
        _cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::SURFACE ||
        _cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::EMPTY) {
    ...
    if (_cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::FLUID ||
        _cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::SURFACE ||
        _cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::EMPTY) {
    ...
    for (int j = 1; j < _domain.size_y + 1; ++j) {
        for (int i = 1; i < _domain.size_x + 1; ++i) {
            ...
            if (_cells(i, j).type() != cell_type::FLUID && _cells(i, j).type() != cell_type::EMPTY) {
                if (_cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::FLUID &&
            ...
            if (_cells(i, j).type() == cell_type::FLUID) {
                if (_cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::TOP)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::RIGHT)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::EMPTY) {
                ...
                if (_cells(i, j).neighbour(border_position::RIGHT)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::EMPTY) {
                    _cells(i, j).set_cell_type(cell_type::SURFACE);
                ...
                if (_cells(i, j).neighbour(border_position::TOP)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::EMPTY) {
                    _cells(i, j).set_cell_type(cell_type::SURFACE);
                ...
                if (_cells(i, j).neighbour(border_position::BOTTOM)->type() == cell_type::EMPTY &&
                    _cells(i, j).neighbour(border_position::LEFT)->type() == cell_type::EMPTY) {
                    _cells(i, j).set_cell_type(cell_type::SURFACE);
                    _cells(i, j).add_border(border_position::SOUTHWEST);
                    _surface_cells.push_back(&_cells(i, j));
...
void Grid::reset_fluid_cells() {
    ...
    std::sort(_fluid_cells.begin(), _fluid_cells.end(), [this](Cell *lhs, Cell *rhs) {
        return lhs->i() + lhs->j() * _domain.imax < rhs->i() + rhs->j() * _domain.imax;
    });
    ...
    for (auto &elem : _surface_cells) {
        _fluid_cells.erase(std::remove(_fluid_cells.begin(), _fluid_cells.end(), elem), _fluid_cells.end());
    }
}
...
void Grid::check_geometry_file(std::vector<std::vector<int>> &geometry_data) {
    for (int j = _domain.jmin + 1; j < _domain.jmax - 1; ++j) {
        for (int i = _domain.imin + 1; i < _domain.imax - 1; ++i) {
            if (geometry_data.at(i).at(j) == 3 || geometry_data.at(i).at(j) == 4 || geometry_data.at(i).at(j) == 5) {
                int sum = geometry_data.at(i + 1).at(j) + geometry_data.at(i - 1).at(j) +
...
const std::vector<std::vector<int>> &Grid::get_geometry_excluding_ghosts() const { return geometry_excluding_ghosts; }
